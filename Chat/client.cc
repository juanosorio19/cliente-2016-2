...
void playSound(message &msg, SoundBuffer &buffer) {
  ...
  while (sound.getStatus() == sf::Sound::Playing) {
    ...
    cout << "\rPlaying... " << sound.getPlayingOffset().asSeconds()
         << " sec        ";
    cout << flush;
  }
...
void sendVoice(const SoundBuffer &buffer, vector<string> &tokens,
               socket &s) {  //, string userName , Time &time1 ??
...
// message &msg,
void voiceCall(SoundBuffer &buffer, bool &callState,
               SoundBufferRecorder &recorder, vector<string> &tokens,
               socket &s) {
...
void handleInputFromServer(message &msg, SoundBuffer &buffer, bool &callState,
                           thread *listening, SoundBufferRecorder &recorder,
  ...
  if (action == "voice" and callState != true) {
    ...
    listening = new thread(voiceCall, ref(buffer), ref(callState),
                           ref(recorder), ref(tokens), ref(s));
    ...
    msg >> idSender;
    cout << action << " says: " << idSender << endl;
  }

  // si es de otro tipo solamente muestremlo
}

void handleInputFromConsole(SoundBuffer &buffer, SoundBufferRecorder &recorder,
                            vector<string> &tokens, socket &s, thread *speaking,
                            bool &callState) {
  if (tokens[0] == "voice") {
  ...
  while (true) {
    if (poll.poll()) {  // There are events in at least one of the sockets
      if (poll.has_input(s)) {
        ...
        handleInputFromServer(m, receive_buffer, callState, listening, recorder,
                              tokens, s);
      ...
      if (poll.has_input(console)) {
        ...
        handleInputFromConsole(buffer, recorder, tokens, s, speaking,
                               callState);
      }
    }
  }
