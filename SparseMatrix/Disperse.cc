...
void concurrentMult(const SparseMatrix<int> &a, const vector<int> b,
                    SparseMatrix<int> &result, const int col,
...
void mult(const SparseMatrix<int> &a, const SparseMatrix<int> &b,
          SparseMatrix<int> &result) {
  // Multiplica matriz a con la matriz b
  vector<SparseMatrix<int>> matrices(
      b.getCols(), SparseMatrix<int>(a.getRows(), b.getCols()));
  ...
  for (int j = 0; j < b.getCols(); j++) {
    ...
    // cout<<"pase la prueba\n";
    auto w = [&a, temporal, &result, j, &matrices]() {
