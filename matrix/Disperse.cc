...
void concurrentMult(SparseMatrix<int>& a, vector<int>& b,SparseMatrix<int> &result,int col) {
    ...
    for(int i=0;i<a.getRows();i++){
      for(int j=0;j<a.getCols();j++){
        ...
        for(int k=0;k<b.size();k++){
          if((a.get(i,k)and b[k] ) !=0 ) result.set(result.get(i,col)+(a.get(i,k)*b[k]),i,col);
        }
      }
    }
}
void mult(SparseMatrix<int>& a, SparseMatrix<int>& b,SparseMatrix<int> &result) {
  ...
  // Multiplica matriz a con la matriz b
  {
    ...
    for(int j=0;j<b.getRows();j++){
      ...
      auto w = [&a, &temporal, &result,j]() { concurrentMult(a, temporal, result, j); };
      pool.submit(w);
