...
public:
  ...
  Matrix(const string &file) {
    double iterator; // file contains an undetermined number of integer values
    ...
    bool flag = true;
    while (flag) // if not at end of file, continue reading numbers
    {
      for (int m = 1; m <= rows; ++m) {
        for (int n = 1; n <= cols; ++n) {
          fin >> iterator;
          if (iterator == -1)
            data[m][n] = numeric_limits<double>::max();
          else
            data[m][n] = iterator;
        }
      }
  ...
  void setData(int i, int j, double number) { data[i][j] = number; }
...
void mult(Matrix &a, Matrix &b, Matrix &result, const int &index,
          vector<bool> &threadState, int count) {
  // Assertion missed, this->cols and b.rows have to be equals
  ...
  for (int i = 1; i <= a.getRows(); i++) {
    for (int j = 1; j <= b.getCols(); j++) {
      ...
      for (int k = 1; k <= a.getCols(); k++) {
        // result.data[i][index]=result.data[i][index]+
        // (this->data[i][k]*b.data[k][j]);
        result.setData(i, index, (result.getData(i, index) +
                                  (a.getData(i, k) * b.getData(k, j))));
      }
    }
  }
...
Matrix threadMult(Matrix &a, Matrix &b) {
  ...
  for (int j = 1; j <= b.getCols(); j++) {
    ...
    if (!threadState[count]) {
      ...
      threads[count] = async(launch::async, mult, ref(a), ref(temporal),
                             ref(result), ref(j), ref(threadState), count);
      threads[count].get();
      count++;
    }
    ...
    // threads[j]= thread (mult,ref(a),ref(temporal),ref(result),ref(j));
    // //making a* b.col(j)
    // cout<<"asi vamos \n";
    // result.printMatrix();
    // threads[j].join();
  ...
  /*	for(int i=1;i<=numCPU;++i){
                  if (threads[count].wait_for(std::chrono::seconds(0))!=
     future_status::ready ) {
                          threads[i].wait();
